...
## Phase 13 — v1.2 competitive feature expansion *(complete, 2026-06-19 → 2026-06-20)*
...
- [x] **Image trace → CustomChord layer** *(complete, 2026-06-20)*. New `core/include/caustic/image_trace.hpp` (header-only, no raylib dep — tests drive it with hand-built synthetic buffers). Pipeline: **Sobel `|Gx| + |Gy|` magnitude** → **stratified sampling** (one strongest pixel per `grid_divisions × grid_divisions` cell above `edge_threshold`) → **clamp to `max_nails`** by edge strength → scan-line stable sort → pixel→math-up world coords in `[-1, 1]` with aspect ratio preserved → chord rule dispatch. Three chord rules: `Modular` (chord *i* → `round(k·i) mod N`, skips self-loops), `Sequential` (i→i+1, N−1 chords), `Nearest` (each nail to its `nearest_k` neighbours, stored as `(lo, hi)` and globally de-duplicated). **GUI:** "Image trace" section in the CustomChord param panel — four sliders (`max nails`, `grid divisions`, `edge threshold`, conditional `modular k` / `nearest k`), a rule combo, and an **Import image…** button that opens the native picker (PNG / JPEG / BMP filter), auto-downscales runaway-large images to 1024 px long-side, converts to grayscale via raylib, runs the trace, pushes the result through the existing CustomChord undo stack so `Ctrl+Z` rescues a bad trace, and replaces the layer's nails + chords. **The angle that distinguishes Caustic from photo-string-art tools** stays intact — the output is a normal Caustic preset the user can keep editing, not a peg-board build script. 13 new doctest cases (synthetic step / diagonal edges, threshold reject, tiny / null inputs, per-rule invariants, determinism, `max_nails` cap). 215 doctest total (was 202); 36 CTest unchanged.
- [x] **Visual timeline editor for Keyframed envelope** *(complete, 2026-06-20)*. New `keyframed_timeline_editor` widget in `app/main.cpp` replaces the table-of-rows UI with a ~220-tall 2D curve view: t on x (0..1), value on y in the user's `[y_min, y_max]` (flipped so up = high). Background + t-grid (every 0.1 or every 1/16 when snap is on) + value-grid + zero-axis highlight. Piecewise-linear curve between keys plus faded held-value pre-roll (t=0 → first key) and tail (last key → t=1) so the user sees the evaluator's edge-clamp. Vertical playback indicator at `anim.current_t` shows where the scrub is. Drag a key to move it (clamped to `(prev.t + ε, next.t − ε)` so the sort order can't invert mid-drag); left-click empty canvas adds a new key at the cursor (snap honoured) and enters drag; right-click on a key deletes it (won't drop below 2 keys — degenerate-envelope guard). Optional snap to 1/16 t and 1/20 of the visible Y range. Tooltip on hovered/dragged key shows `t` / `value`. Aux controls: y_min / y_max sliders, snap checkbox, **Auto-fit Y** (sets range from current keys with 15% pad), **Reset keys** (back to `{(0,0), (1,1)}`). **The underlying `Keyframed { vector<pair<t, value>> }` data structure is unchanged** — pure UX, as the deliverable spec required. Test counts unchanged (215 doctest + 36 CTest) because the existing `test_envelope.cpp` Keyframed coverage isn't affected and the editor itself is pure UI.
...
**Acceptance:** A new user can hit "Surprise me" on a fresh launch and land on a recognisable canonical pattern (not noise) for each generator; a user can copy a preset URL to a friend and have them paste it into the app to see the same scene; the CLI emits valid G-code that an AxiDraw owner can plot directly without a `vpype` detour; an image dropped onto the canvas creates a CustomChord layer that visibly traces the input's strong edges; the Keyframed envelope's animation panel shows a draggable 2D curve and the underlying data still round-trips through the existing JSON schema. **All five deliverables shipped over two days (2026-06-19 → 2026-06-20).** Doctest 159 → 215 (+56); CTest 33 → 36 (+3).
