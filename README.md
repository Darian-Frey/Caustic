...
> **Why this status:** Phases 0–11 closed plus v1.1 polish (extra generators, scatter attractors, LinearEnvelope drag editor, Keyframed envelope, PNG/GIF/mp4 bake, CustomChord editor, polar grid, per-chord stroke, preset thumbnails, universal pan, keyboard shortcuts, IDE-style draggable-sidebar layout, multi-format static export). **Phase 13 complete** (v1.2 competitive features — all 5 shipped: per-generator "Surprise me" randomiser; shareable preset URLs; direct G-code / HPGL output for pen plotters; image trace → CustomChord layer; visual timeline editor for the Keyframed envelope). 17 generators across three pipeline tiers; multi-layer scenes; animation system; SVG/PNG/JPEG/G-code/HPGL static export plus SVG/PNG-sequence/GIF/mp4 animation bake; headless CLI. Only Phase 12 (README screenshots, license, Windows cross-compile, itch.io page) remains for the public 1.0.
...
- **Shareable preset URLs** — single-line `caustic:p1:<base64url>` strings (~0.9–5.6 KB for the bundled gallery) you can paste into chat / email / forums. Copy URL / Paste URL buttons in the Presets tab use the system clipboard. No server, no scheme handler required.
- **Direct G-code / HPGL output for pen plotters** — emit Grbl-flavour G-code (configurable pen-up / pen-down Z, travel / draw / plunge feedrates) or HPGL (`SP` pen-select, `PU` / `PD` motion, integer plotter units at 40 PU/mm) without round-tripping through `vpype` or the Inkscape AxiDraw plugin. Available in the GUI Export panel and via `caustic-cli --format gcode|hpgl` (extension inference on `.gcode` / `.nc` / `.gc` / `.hpgl` / `.plt`).
- **Image trace → CustomChord layer** — drop a PNG / JPEG / BMP onto the **Import image…** button in the CustomChord panel. Sobel edge detection + stratified sampling lays nails on the strongest edges, then connects them with a configurable rule (`modular k` / `sequential` / `nearest k`). The result is a normal Caustic preset you can keep editing in the nail editor — not a peg-board build script.
- **Visual timeline editor for the Keyframed envelope** — drag keys on a 2D curve (t on x, value on y), left-click empty canvas to add, right-click a key to delete, snap-to-grid toggle, **Auto-fit Y** + **Reset keys** buttons, vertical playback indicator at the current animation `t`. The faded held-value pre-roll and tail show the evaluator's edge-clamp behaviour.
