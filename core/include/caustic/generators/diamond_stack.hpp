...
// Which fan set to emit per module. Mirrors DiamondStackFans in preset.hpp
// but defined as plain ints here to keep the generator header free of any
// preset-layer dependency. The factory translates the enum on the way in.
//   0 = Both       (4 fans per module)
//   1 = Vertical   (upper + lower only — apexes at top/bottom tips)
//   2 = Horizontal (left + right only — apexes at waist points)
inline ChordSet diamond_stack(int n_modules, int N,
    ...
    const bool want_vert  = (fans == 0 || fans == 1);
    const bool want_horiz = (fans == 0 || fans == 2);
    const int  fans_per_module = (want_vert ? 2 : 0) + (want_horiz ? 2 : 0);

    ChordSet chords;
    chords.reserve(static_cast<std::size_t>(n_modules) *
                   static_cast<std::size_t>(fans_per_module) *
    ...
    raw.reserve(static_cast<std::size_t>(n_modules) *
                static_cast<std::size_t>(fans_per_module) *
                static_cast<std::size_t>(N));
    ...
    for (int m = 0; m < n_modules; ++m) {
        ...
        if (want_horiz) {
            emit_fan(L, T, B);  // left  parabola, vertex at left  waist
            emit_fan(R, T, B);  // right parabola, vertex at right waist
        }
    }
    ...
    for (std::size_t i = 0; i < raw.size(); ++i) {
        chords.push_back({raw[i].first, raw[i].second,
                          total > 0.0 ? static_cast<double>(i) / total : 0.0});
    }
